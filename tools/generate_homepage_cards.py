...
def main():
    ...
    for c in cards:
        ...
        out.append(
            '\n      <a class="ep-card" href="episodes/%s.html" data-ep-slug="%s"'
            ' style="--hf:%scqw;--nf:%scqw">'
...
SERIES_PAGE = {
    "Navigators": "navigators", "Sky Gods": "sky-gods", "Living the Dream": "living-the-dream",
    "World Cups": "world-cups", "Risk vs Reward": "risk-vs-reward",
    "Resources, Tools and Tips": "resources-tools-tips", "Weather Patterns": "weather-patterns",
    "Brand Stories": "brand-stories", "Storytellers": "storytellers",
    "The Dark Side": "the-dark-side", "Flight Mechanics": "flight-mechanics",
    "New Technologies": "new-technologies", "Know Your Equipment": "know-your-equipment",
}


def _write_modal_payload(cards):
    """The popup data for these fifteen cards, and only these fifteen.

    The homepage opens the same card the knowledge base and the library open, so
    a click shows the episode before committing to the page. It does NOT reuse
    library-episodes.js: that holds all 86 at 163KB, and the homepage needs 15.
    """
    ...
    for c in cards:
        ...
        d = KBD.modal_data(e["title"], e.get("guest", ""), s,
                           "knowledge-base/%s.html" % SERIES_PAGE.get(s, ""))
        if d is None:
            continue
        d["page"] = "episodes/%s.html" % c["slug"]
        d["seriesPage"] = ("knowledge-base/%s.html" % SERIES_PAGE[s]) if s in SERIES_PAGE else ""
        d["libRoot"] = True          # index.html is at the site root
        out[c["slug"]] = d
    body = ("/* Generated by tools/generate_homepage_cards.py. Do not edit. */\n"
            "window.LIB_MODAL = " + json.dumps(out, ensure_ascii=False, sort_keys=True) + ";\n")
    open(os.path.join(ROOT, "home-episodes.js"), "w", encoding="utf-8").write(body)
    if len(out) != len(cards):
        print("  WARNING: popup data for only %d of %d cards" % (len(out), len(cards)))
