...
(function () {
  ...
  /* v2: the lean runs only while there is somewhere to go (it used to run
     every frame for the life of the page, on screen or not) */
  var leaning = false;
  function wake() { if (!leaning && !calm.matches) { leaning = true; requestAnimationFrame(frame); } }
  ...
  root.addEventListener('pointerleave', function () { wantX = 0; wantY = 0; wake(); });
  ...
  function frame() {
    ...
    if (Math.abs(wantX - haveX) > 0.001 || Math.abs(wantY - haveY) > 0.001) requestAnimationFrame(frame);
    else leaning = false;
  }
  ...
  layout();
  drift();
  if (!calm.matches) { leaning = true; frame(); }
})();
